...
export function insertAdjacentFSGHTML(position: InsertPosition, html: string): void {
  // Utils.insertHTMLAndExtractStyle(FavoritesSearchGalleryContainer.container, position, html);
}

export function insertAdjacentFSGElement(position: InsertPosition, element: HTMLElement): void {
  FAVORITES_SEARCH_GALLERY_CONTAINER.insertAdjacentElement(position, element);
}
