import {awesompleteIsSelected, awesompleteIsVisible, hideAwesomplete} from "../../utils/dom/awesomplete";
...
import {debounceAfterFirstCall} from "../../utils/misc/generic";
import {openSearchPage} from "../../utils/dom/links";

const SEARCH_BOX: HTMLTextAreaElement | HTMLInputElement = document.createElement("textarea");
...
function addEventListenersToSearchBox(): void {
  ...
  SEARCH_BOX.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Enter") {
      if (!event.repeat && !awesompleteIsSelected(SEARCH_BOX)) {
...
export function setupSearchBox(): void {
  SEARCH_BOX.id = ID;
  SEARCH_BOX.placeholder = "Search Favorites";
  SEARCH_BOX.spellcheck = false;
  SEARCH_BOX.value = SEARCH_HISTORY.lastEditedQuery;

  const parent = document.getElementById(PARENT_ID);

  if (parent !== null) {
    parent.insertAdjacentElement("beforeend", SEARCH_BOX);
  }
