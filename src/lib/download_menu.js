class FavoritesDownloadController {
  ...
  constructor() {
    ...
    this.downloader = new PostDownloader();
  ...
  async downloadFavorites(favorites) {
    ...
    const onFetch = () => {
      totalCount += 1;
      currentBatchCount += 1;
      totalProgressRow.textContent = `Downloading: ${totalCount} / ${postCount}`;
    };
    ...
    for (let i = 0; i < batches.length; i += 1) {
      ...
      await this.downloader.downloadPosts(batch, onFetch);
    }
    this.clearStatusTextRows();
