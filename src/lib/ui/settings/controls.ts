import { MultiSelectSetting, SelectSetting, SliderSetting, StepperSetting, ToggleSetting } from "@/lib/ui/settings/setting";
...
export const segmented = <T extends string | number>(config: Partial<SelectSetting<T>>): SettingsControl => (): HTMLElement => buildSegmentedRow(config);
export const multiSegmented = <T extends number>(config: Partial<MultiSelectSetting<T>>): SettingsControl => (): HTMLElement => buildMultiSegmentedRow(config);
