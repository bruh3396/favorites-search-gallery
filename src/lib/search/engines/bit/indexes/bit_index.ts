...
export class BitIndex<Doc> {
  ...
  private place(doc: Doc, position: number): string[] {
    ...
    for (const term of this.termsFor(doc)) {
      const sharedTerm = internString(term);
      const positions = this.positionsByTerm.get(sharedTerm);

      if (positions === undefined) {
        this.positionsByTerm.set(sharedTerm, [position]);
        newTerms.push(sharedTerm);
      } else {
