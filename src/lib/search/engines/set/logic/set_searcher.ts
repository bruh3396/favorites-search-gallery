...
import { SearchQuery } from "@/lib/search/engines/set/logic/search_query";
...
export class SetSearcher<Doc> {
  ...
  private docsWithAll(terms: AbstractSearchTerm[]): ReadonlySet<Doc> {
    ...
    const docSets = terms.map(term => this.resolver.resolve(term)).sort((a, b) => a.size - b.size);
  ...
  private docsWithAny(terms: AbstractSearchTerm[]): ReadonlySet<Doc> {
    if (terms.length === 1) {
      return this.resolver.resolve(terms[0]);
    }
    const docs = new Set<Doc>();

    for (const term of terms) {
      this.resolver.resolve(term).forEach(doc => docs.add(doc));
    }
    return docs;
  }
}
