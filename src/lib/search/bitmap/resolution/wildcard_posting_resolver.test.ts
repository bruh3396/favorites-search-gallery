...
import { Posting } from "@/lib/search/bitmap/postings/posting";
...
interface Harness {
  ...
  resolver: WildcardPostingResolver<Doc>;
  unions: number;
...
function harness(docs: Doc[] = corpus): Harness {
  ...
  const unionSpy = vi.spyOn(bitmapIndex, "unionOfPostings");
  const resolver = new WildcardPostingResolver(bitmapIndex);
