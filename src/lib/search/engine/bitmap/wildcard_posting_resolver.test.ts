...
// One doc per tag so a resolved posting maps cleanly back to a tag name.
const corpus: Doc[] = [
  doc("banana", "banana"),
  doc("bandana", "bandana"),
  doc("cabana", "cabana"),
...
describe("WildcardPostingResolver", () => {
  ...
  test("resolves a multi-star wildcard through the regex path", () => {
    // b*na -> starts with b, ends with na
...
describe("WildcardPostingResolver caching", () => {
  ...
  test("keys the cache per wildcard shape", () => {
    ...
    // three distinct shapes -> three unions, none shared
  ...
  test("caches the empty result without re-resolving", () => {
    ...
    expect(h.resolver.resolve(term)).toBeUndefined();
    expect(h.unions).toBe(0); // no terms matched, so unionize is never called
  });
  ...
  test("index() invalidates the cached union", () => {
    ...
    // after re-indexing, the same pattern must recompute rather than serve a stale posting
    expect(h.resolveIds("ban*")).toEqual(["banana", "bandana"]);
