import { ExpressionContext, SearchExpression } from "@/lib/search/engine/bitmap/search_expression";
...
function idsFor(items: Item[], expression: SearchExpression): string[] {
  return expression.search(contextFor(items)).map(doc => doc.id).sort();
}
...
describe("SearchExpression arbitrary nesting", () => {
  ...
  test("evaluates the deeply nested example from the query language discussion", () => {
    // small AND ( sweet OR ( big AND ( red OR green ) ) )
