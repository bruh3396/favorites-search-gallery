...
function expand(query: string, vocabulary: string[]): { andTerms: string[]; orGroups: string[][]; isUnmatchable: boolean } {
  ...
  const expander = new WildcardTermExpander<Fruit>(matcher);
...
describe("expandWildcards", () => {
  ...
  test("a prefix wildcard becomes an or group of matching terms", () => {
    expect(expand("cat*", ["cat", "catgirl"])).toEqual({ andTerms: [], orGroups: [["cat", "catgirl"]], isUnmatchable: false });
  });

  test("a single matching term flattens to an and term", () => {
    expect(expand("cat*", ["catgirl"])).toEqual({ andTerms: ["catgirl"], orGroups: [], isUnmatchable: false });
  });

  test("a substring wildcard expands via containing terms", () => {
    expect(expand("*an*", ["banana", "canvas"])).toEqual({ andTerms: [], orGroups: [["banana", "canvas"]], isUnmatchable: false });
  });

  test("a suffix wildcard expands via ending terms", () => {
    expect(expand("*girl", ["catgirl", "doggirl"])).toEqual({ andTerms: [], orGroups: [["catgirl", "doggirl"]], isUnmatchable: false });
  });

  test("a multi star wildcard expands via matching terms", () => {
    expect(expand("c*t*", ["carrot", "cat"])).toEqual({ andTerms: [], orGroups: [["carrot", "cat"]], isUnmatchable: false });
  });
  ...
  test("a negated wildcard expands to negated and terms", () => {
    expect(expand("-cat*", ["cat", "catgirl"])).toEqual({ andTerms: ["-cat", "-catgirl"], orGroups: [], isUnmatchable: false });
  });

  test("a wildcard inside an or group flattens into the group", () => {
    expect(expand("( cat* ~ dog )", ["cat", "catgirl"])).toEqual({ andTerms: [], orGroups: [["dog", "cat", "catgirl"]], isUnmatchable: false });
  });

  test("multiple wildcard kinds inside an or group all flatten", () => {
    expect(expand("( *cat* ~ dog* )", ["cat", "catgirl", "dog", "doghouse"])).toEqual({ andTerms: [], orGroups: [["dog", "doghouse", "cat", "catgirl"]], isUnmatchable: false });
  });

  test("keeps a required exact term alongside an expanded wildcard", () => {
    expect(expand("solo cat*", ["cat", "catgirl"])).toEqual({ andTerms: ["solo"], orGroups: [["cat", "catgirl"]], isUnmatchable: false });
  });

  test("a negated wildcard inside an or group is dropped from the group", () => {
    expect(expand("( -cat* ~ dog )", ["cat", "catgirl"])).toEqual({ andTerms: ["dog"], orGroups: [], isUnmatchable: false });
  });

  test("an or group of only a negated wildcard is unmatchable", () => {
    expect(expand("( -cat* ~ -dog* )", ["cat", "dog"]).isUnmatchable).toBe(true);
  });
});
