...
import { correctTagsIfInvalid } from "@/features/favorites/model/enrichment/tag_corrector";
...
describe("correctTagsIfInvalid", () => {
  test("returns false when tags already match (post.id present on favorite)", () => {
    ...
    expect(correctTagsIfInvalid(favorite, post())).toBe(false);
  });
  ...
  test("returns true when a tag is missing on the favorite", () => {
    ...
    expect(correctTagsIfInvalid(favorite, post())).toBe(true);
  });
  ...
  test("writes corrected sorted tags to post.tags when correction is needed", () => {
    ...
    correctTagsIfInvalid(favorite, p);
  ...
  describe("media tag correction", () => {
    test("adds 'video' for mp4 files", () => {
      ...
      expect(correctTagsIfInvalid(favorite, p)).toBe(true);
    ...
    test("favorite already carrying 'video' for an mp4 needs no correction", () => {
      ...
      expect(correctTagsIfInvalid(favorite, p)).toBe(false);
    });
