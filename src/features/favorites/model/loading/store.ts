...
const database = new Database<StoredPost>("FavoritesV2", `user${ON_FAVORITES_PAGE ? FAVORITES_PAGE_ID : USER_ID}`);
const databaseUpdater = new CoalescingExecutor<StoredPost>(100, 1_000, database.update.bind(database));
let isDatabaseEmpty = true;

export async function writeAll(favorites: Favorite[]): Promise<void> {
  await database.write([...favorites].reverse().map(favorite => postToStoredPost(favorite.post)));
  isDatabaseEmpty = false;
}

export function update(favorite: Favorite): void {
  if (!isDatabaseEmpty) {
    databaseUpdater.schedule(postToStoredPost(favorite.post));
  }
}
...
export async function readAll(): Promise<Post[]> {
  ...
  return records.map(storedPostToPost);
}
