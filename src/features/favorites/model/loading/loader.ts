...
export function readDatabaseFavorites(getAdditionalTags: (id: string) => string | undefined): Promise<FavoriteItem[]> {
  return FavoritesStore.readAll().then(records => records.map(r => new FavoriteItem(r, getAdditionalTags(r.id))));
}
