class PostHTMLElement {
  ...
  static createHTMLTemplates() {
    ...
    PostHTMLElement.createDownloadButtonHTMLTemplate();
    PostHTMLElement.createPostHTMLTemplate();
  }

  static createRemoveFavoriteButtonHTMLTemplate() {
    PostHTMLElement.removeFavoriteButtonHTML = `<img class="remove-favorite-button utility-button" src=${Utils.createObjectURLFromSvg(SVGIcons.heartMinus)}>`;
  }

  static createAddFavoriteButtonHTMLTemplate() {
    PostHTMLElement.addFavoriteButtonHTML = `<img class="add-favorite-button utility-button" src=${Utils.createObjectURLFromSvg(SVGIcons.heartPlus)}>`;
  }

  static createDownloadButtonHTMLTemplate() {
    PostHTMLElement.downloadButtonHTML = Flags.onDesktopDevice ? `<img class="download-button utility-button" src=${Utils.createObjectURLFromSvg(SVGIcons.download.replace("FFFFFF", "0075FF"))}>` : "";
  }

  static createPostHTMLTemplate() {
    const addOrRemoveButtonHTML = Flags.userIsOnTheirOwnFavoritesPage ? PostHTMLElement.removeFavoriteButtonHTML : PostHTMLElement.addFavoriteButtonHTML;
    ...
    PostHTMLElement.htmlTemplate.innerHTML = `
        <${containerTagName}>
          <img>
          ${addOrRemoveButtonHTML}
          ${PostHTMLElement.downloadButtonHTML}
          ${canvasHTML}
        </${containerTagName}>
    `;
  }
