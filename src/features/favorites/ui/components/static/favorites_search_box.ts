...
function createMobileSearchBar(): HTMLInputElement {
  const html = `
...
function createMobileSymbolRow(): void {
  ...
  for (const button of Array.from(symbolContainer.querySelectorAll("button"))) {
    ...
    button.addEventListener("click", (): void => {
      ...
      const selectionStart = SEARCH_BOX.selectionStart ?? 0;

      SEARCH_BOX.value = value.slice(0, selectionStart) + button.textContent + value.slice(selectionStart);
      // this.updateVisibilityOfSearchClearButton();
      SEARCH_BOX.selectionStart = selectionStart + 1;
      SEARCH_BOX.selectionEnd = selectionStart + 1;
      SEARCH_BOX.focus();
    }, {
      passive: true
    });
  ...
  SEARCH_BOX.addEventListener("blur", async() => {
    await sleep(10);
...
export function setupFavoritesSearchBox(): void {
  SEARCH_BOX = ON_MOBILE_DEVICE ? createMobileSearchBar() : createDesktopSearchBar();
