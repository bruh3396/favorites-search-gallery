import { Fruit, allDocNames, allTerms, engine, fruitDocs, index } from "@/tests/lib/search/fixtures/fruit_search";
...
describe("filter", () => {
  ...
  test("all tags", () => {
    ...
    testSearch(orAllQuery, allDocNames);
    testSearch(andAllQuery, []);

    for (const tag of allTerms) {
      testSearch(tag, fruitDocs.filter(item => item.tags.has(tag)).map(item => item.name));
      testSearch(`-${tag}`, fruitDocs.filter(item => !item.tags.has(tag)).map(item => item.name));
    }
  });
