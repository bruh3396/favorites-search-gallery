class ImageUtils {
  ...
  static async getImageExtensionFromThumb(thumb) {
    ...
    if (Flags.onFavoritesPage) {
      extension = await ImageUtils.fetchValidImageExtension(thumb);
    } else {
  ...
  static async fetchImageExtension(id) {
    ...
    Extensions.setFromAPIPost(apiPost);
   ...
   * @returns {Promise<MediaExtension>}
   */
  static async fetchValidImageExtension(thumb) {
    const apiPost = await APIPost.fetch(thumb.id);

    if (apiPost.isEmpty) {
      return this.tryAllPossibleExtensions(thumb);
    }
    Extensions.setFromAPIPost(apiPost);
    return apiPost.extension;
  }
