class Utils {
   ...
   * @param {Boolean} value
   * @param {String} name
   * @param {{important?: Boolean, onValue?: String, offValue?: String}} options
   */
  static toggleDisplay(selectors, value, name, options = {}) {
    const important = options.important ? " !important" : "";
    const onValue = options.onValue || "block";
    const offValue = options.offValue || "none";
    const selectorString = selectors.join(", ");
    const display = value ? onValue : offValue;
    const declaration = `display: ${display}${important}`;
    const rule = `${selectorString} {${declaration}}`;

    Utils.insertStyleHTML(rule, name);
  }
   ...
   * @param {String} name
   */
  static toggleVisibility(selectors, value, name) {
    const selectorString = selectors.join(", ");
    const visibility = value ? "" : "hidden";
    const declaration = `visibility: ${visibility}`;
    const rule = `${selectorString} {${declaration}}`;
  ...
  static async findImageExtensionsOnSearchPage(pageNumber = undefined) {
    ...
    Array.from(dom.getElementsByTagName("post"))
      ...
      .forEach(apiPost => Extensions.setFromAPIPost(apiPost));
  }
  ...
  static async download(id) {
    ...
    const downloadRequest = await post.getDownloadRequest();
    const blob = await downloadRequest.blob();

    Utils.downloadBlob(blob, downloadRequest.filename);
  }
